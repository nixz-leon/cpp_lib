...
*/
template <typename T>
void fftc1(T f[], int N, int skip, int forward) {

	int b, index1, index2, trans_size, trans; 
	T pi2 = 4.*asin(1.); 
	T pi2n, cospi2n, sinpi2n; /* Used in recursive formulas for Re(W^b) and Im(W^b) */
	struct ecomplex<T> wb; /* wk = W^k = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
	struct ecomplex<T> temp1, temp2; /* Buffers for implementing recursive formulas */
	struct ecomplex<T> *c = (struct ecomplex<T> *)f; /* Treat f as an array of N ecomplex numbers */
	...
	for(trans_size= 2; trans_size <= N; trans_size*= 2) { /* trans_size = size of transform being computed */
		pi2n = forward * pi2 / (T)trans_size; /* +- 2 pi/trans_size */
...
*/
template<typename T>
void fftr1(T f[], int N, int forward) {

	int b; 
	T pi2n = 4. * asin(1.) / N, cospi2n= cos(pi2n), sinpi2n= sin(pi2n); /* pi2n = 2 Pi/N */
	struct ecomplex<T> wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
	struct ecomplex<T> temp1, temp2; /* Buffers for implementing recursive formulas */
	struct ecomplex<T> *c = (struct ecomplex<T> *)f; /* Treat f as an array of N/2 ecomplex numbers */
...
template <typename T>
void fftrnd(T f[], int ndims, int isize[], int forward) {
	...
	struct ecomplex<T> wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
	struct ecomplex<T> temp1, temp2; /* Buffers for implementing recursive formulas */
	struct ecomplex<T> *c = (struct ecomplex<T> *)f; /* Treat f as array of ecomplex numbers */
	...
	for(index= 0; index < totalsize; index+= N) { /* Loop over all but first array index */
		...
		for(j= 1; indices[j] == size[j]-1 && j < ndims; j++) { /* If the leftmost indices are maximal reset them to 0. 
